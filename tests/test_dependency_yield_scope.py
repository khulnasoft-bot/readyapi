...
import pytest
from readyapi import APIRouter, Depends, HTTPException, ReadyAPI
...
def test_app_level_dep_scope_function() -> None:
    app = ReadyAPI(dependencies=[Depends(raise_after_yield, scope="function")])
...
def test_app_level_dep_scope_request() -> None:
    app = ReadyAPI(dependencies=[Depends(raise_after_yield, scope="request")])
