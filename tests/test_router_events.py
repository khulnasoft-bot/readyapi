...
def test_router_nested_lifespan_state(state: State) -> None:
    @asynccontextmanager
    async def lifespan(app: ReadyAPI) -> AsyncGenerator[dict[str, bool], None]:
        state.app_startup = True
        yield {"app": True}
        state.app_shutdown = True

    @asynccontextmanager
    async def router_lifespan(app: ReadyAPI) -> AsyncGenerator[dict[str, bool], None]:
        state.router_startup = True
        yield {"router": True}
        state.router_shutdown = True

    @asynccontextmanager
    async def subrouter_lifespan(
        app: ReadyAPI,
...
def test_merged_no_return_lifespans_return_none() -> None:
    ...
    @asynccontextmanager
    async def router_lifespan(app: ReadyAPI) -> AsyncGenerator[None, None]:
        yield
...
def test_merged_mixed_state_lifespans() -> None:
    ...
    @asynccontextmanager
    async def sub_router_lifespan(app: ReadyAPI) -> AsyncGenerator[None, None]:
        yield
...
def test_router_sync_generator_lifespan(state: State) -> None:
    ...
    def lifespan(app: ReadyAPI) -> Generator[None, None, None]:
        state.app_startup = True
        yield
        state.app_shutdown = True

    app = ReadyAPI(lifespan=lifespan)  # type: ignore[arg-type]
...
def test_startup_shutdown_handlers_as_parameters(state: State) -> None:
    """Test that startup/shutdown handlers passed as parameters to ReadyAPI are called correctly."""
    ...
    app = ReadyAPI(on_startup=[app_startup], on_shutdown=[app_shutdown])
