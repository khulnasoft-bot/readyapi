...
from readyapi.exceptions import DependencyScopeError
...
def _get_signature(call: Callable[..., Any]) -> inspect.Signature:
    try:
        ...
        # Handle type annotations with if TYPE_CHECKING, not used by ReadyAPI
    ...
    for param_name, param in signature_params.items():
        ...
        if add_non_field_param_to_dependency(
            ...
            assert param_details.field is None, (
                f"Cannot specify multiple ReadyAPI annotations for {param_name!r}"
            )
            continue
    ...
    *,
    ...
    if get_origin(use_annotation) is Annotated:
        ...
        if readyapi_specific_annotations:
            readyapi_annotation: FieldInfo | params.Depends | None = (
        ...
        if isinstance(readyapi_annotation, FieldInfo):
            ...
            field_info = copy_field_info(
                field_info=readyapi_annotation,
    ...
    # Get FieldInfo from default value
        ...
        assert field_info is None, (
            "Cannot specify ReadyAPI annotations in `Annotated` and default value"
            f" together for {param_name!r}"
        )
        field_info = value
    ...
    if depends is None and lenient_issubclass(
        ...
        assert field_info is None, (
            f"Cannot specify ReadyAPI annotation for type {type_annotation!r}"
        )
...
async def solve_dependencies(
    *,
    ...
    request_astack = request.scope.get("readyapi_inner_astack")
    assert isinstance(request_astack, AsyncExitStack), (
        "readyapi_inner_astack not found in request scope"
    )
    function_astack = request.scope.get("readyapi_function_astack")
    assert isinstance(function_astack, AsyncExitStack), (
        "readyapi_function_astack not found in request scope"
    )
    values: dict[str, Any] = {}
    errors: list[Any] = []
    if response is None:
