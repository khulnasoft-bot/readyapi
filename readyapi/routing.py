...
from readyapi.dependencies.models import Dependant
...
from readyapi.exceptions import (
    EndpointContext,
    ReadyAPIError,
    ...
    async def app(scope: Scope, receive: Receive, send: Send) -> None:
        ...
        async def app(scope: Scope, receive: Receive, send: Send) -> None:
            ...
            async with AsyncExitStack() as request_stack:
                scope["readyapi_inner_astack"] = request_stack
                async with AsyncExitStack() as function_stack:
                    scope["readyapi_function_astack"] = function_stack
...
class _DefaultLifespan:
    ...
    in Starlette. ReadyAPI keeps it to maintain backward compatibility with
...
def get_request_handler(
    ...
    async def app(request: Request) -> Response:
        ...
        file_stack = request.scope.get("readyapi_middleware_astack")
        assert isinstance(file_stack, AsyncExitStack), (
            "readyapi_middleware_astack not found in request scope"
        )
        ...
        errors: list[Any] = []
        async_exit_stack = request.scope.get("readyapi_inner_astack")
        assert isinstance(async_exit_stack, AsyncExitStack), (
            "readyapi_inner_astack not found in request scope"
        )
...
def get_websocket_app(
    ...
    async def app(websocket: WebSocket) -> None:
        ...
        async_exit_stack = websocket.scope.get("readyapi_inner_astack")
        assert isinstance(async_exit_stack, AsyncExitStack), (
            "readyapi_inner_astack not found in request scope"
        )
        solved_result = await solve_dependencies(
...
class APIRouter(routing.Router):
    ...
    an app in multiple files. It would then be included in the `ReadyAPI` app, or
