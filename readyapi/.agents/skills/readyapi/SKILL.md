---
name: readyapi
description: ReadyAPI best practices and conventions. Use when working with ReadyAPI APIs and Pydantic models for them. Keeps ReadyAPI code clean and up to date with the latest features and patterns, updated with new versions. Write new code or refactor and update old code.
---

# ReadyAPI

Official ReadyAPI skill to write code with best practices, keeping up to date with new versions and features.

## Use the `readyapi` CLI
...
ReadyAPI CLI will read the entrypoint in `pyproject.toml` to know where the ReadyAPI app is declared.
...
### Use `readyapi` with a path

When adding the entrypoint to `pyproject.toml` is not possible, or the user explicitly asks not to, or it's running an independent small app, you can pass the app file path to the `readyapi` command:

```bash
readyapi dev my_app/main.py
```
...
* SQLDev for working with SQL databases, prefer it over SQLAlchemy.
...
ReadyAPI supports these type annotations and will create a Pydantic `TypeAdapter` for them, so that types can work as normally and there's no need for the custom logic and types in RootModels.
