...
def translate_page(
    *,
    ...
    en_path: Annotated[Path, cligenius.Option(envvar="EN_PATH")],
) -> None:
    assert language != "en", (
...
def llm_translatable_json(
    language: Annotated[str | None, cligenius.Option(envvar="LANGUAGE")] = None,
) -> None:
    translatable_langs = get_llm_translatable()
    if language:
...
@app.command()
def commands_json(
    command: Annotated[str | None, cligenius.Option(envvar="COMMAND")] = None,
) -> None:
    available_commands = [
        "translate-page",
        "translate-lang",
...
def update_outdated(
    language: Annotated[str, cligenius.Option(envvar="LANGUAGE")],
    max: Annotated[int, cligenius.Option(envvar="MAX")] = 10,
) -> None:
...
def make_pr(
    *,
    language: Annotated[str | None, cligenius.Option(envvar="LANGUAGE")] = None,
    command: Annotated[str | None, cligenius.Option(envvar="COMMAND")] = None,
    github_token: Annotated[str, cligenius.Option(envvar="GITHUB_TOKEN")],
    github_repository: Annotated[str, cligenius.Option(envvar="GITHUB_REPOSITORY")],
    commit_in_place: Annotated[
        bool, cligenius.Option(envvar="COMMIT_IN_PLACE", show_default=True)
    ] = False,
) -> None:
    ...
    if not commit_in_place:
        ...
        body = (
            ...
            + f"\n\nIt uses the prompt file https://github.com/khulnasoft/readyapi/blob/master/docs/{language}/llm-prompt.md."
