...
KHULNASOFT_COM = "https://readyapi.khulnasoft.com"
...
def _add_lang_code_to_url(url: str, lang_code: str) -> str:
    if url.startswith(KHULNASOFT_COM):
        rel_url = url[len(KHULNASOFT_COM) :]
        if not rel_url.startswith(ASSETS_URL_PREFIXES):
            url = url.replace(KHULNASOFT_COM, f"{KHULNASOFT_COM}/{lang_code}")
    return url
