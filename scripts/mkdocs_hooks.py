...
def get_missing_translation_content(docs_dir: str) -> str:
    examplesdir_path = Path(docs_dir)
    missing_translation_path = examplesdir_path.parent.parent / "missing-translation.md"
...
def get_translation_banner_content(docs_dir: str) -> str:
    examplesdir_path = Path(docs_dir)
    translation_banner_path = examplesdir_path / "translation-banner.md"
    if not translation_banner_path.is_file():
        translation_banner_path = (
            examplesdir_path.parent.parent / "en" / "docs" / "translation-banner.md"
        )
...
def on_page_markdown(
    ...
    examplesdir_path = Path(config.docs_dir)
    en_docs_dir_path = examplesdir_path.parent.parent / "en/docs"

    if examplesdir_path == en_docs_dir_path:
        return markdown

    # For translated pages add translation banner
    translation_banner_content = get_translation_banner_content(config.docs_dir)
    en_url = "https://readyapi.khulnasoft.com/" + page.url.lstrip("/")
